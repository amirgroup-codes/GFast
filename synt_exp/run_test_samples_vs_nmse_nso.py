...
if __name__ == '__main__':
    ...
    for i in range(args.iters):
        ...
        for removal in removal_policies:
            ...
            base_dir = Path(f'../synt_results/q{q}_n{n}_{removal}_S{sparsity}_snr{args.snr}_nso/')
            if removal == 'random':
                all_qs = [get_qs_from_delta_random(delta, q, n) for delta in args.delta]
            ...
            for b1 in range(1, b+1): 
                for repeat in num_delays:
                        ...
                        newfolder = f'iter{i}_delta{delta}_b{b1}_d{repeat}'
                        ...
                        # else:
                        #     nmse.append(nmse_val)
                        print(f'- b = {b1}, d = {repeat} (samples = {result["n_samples"]}): NMSE = {nmse_val}')
            ...
            for qs, delta in zip(all_qs, args.delta):
                ...
                for b1 in range(1, nearest_b + 1):
                    ...
                    #         # else:
                    #         #     nmse_perms.append(nmse_val)
                    #         print(f'- b = {b1}, perm = {j} (samples = {result["n_samples"]}, noise = {noise}): NMSE = {nmse_val}')
    ...
    df.to_csv(f'../synt_results/q{q}_n{n}_results_S{sparsity}_snr{args.snr}_nso.csv')
